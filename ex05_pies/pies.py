...
def sort_results(path_to_competitors: str, path_to_results: str) -> list:
    ...
    """
    result_dict = filter_results(path_to_competitors, path_to_results)
    comp_list = get_competitors_list(path_to_competitors)
    sorted_list = sorted(result_dict.items(), key=lambda i: comp_list.index(i[0]))
    sorted_list = sorted(sorted_list, key=lambda i: i[1], reverse=True)
    return sorted_list
...
def find_average_score(results: dict) -> int:
    ...
    for value in results.values():
        totalval += value
        totalnums += 1
    return math.floor(totalval / totalnums)
...
def write_results_csv(path_to_competitors: str, path_to_results: str, file_to_write: str) -> None:
    ...
    with open(file_to_write, 'w') as f:
        f.write("Place,Name,Result\n")
        for tuple in correct_results:
            f.write(str(iterator) + "," + str(tuple[0]) + "," + str(tuple[1]) + "\n")
            iterator += 1
    f.close()
