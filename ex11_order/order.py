...
class Order:
    ...
    def total_volume(self) -> int:
        ...
        total_volume = 0
        for elem in self.order_items:
            total_volume += elem.total_volume
        return total_volume
...
class Container:
    ...
    def __init__(self, volume: int, orders: list):
        ...
        :param volume: the maximum volume of the container.
        :param orders: the orders in the container.
        """
        self.volume = volume
        self.orders = orders
    ...
    def volume_left(self) -> int:
        """Calculate the volume left in the container.

        :return: int
        """
        orders_volume = 0
        for order in self.orders:
            orders_volume += order.total_volume
        return self.volume - orders_volume
    ...
    def prepare_containers(self, orders: tuple) -> dict:
        ...
        for order in orders:
            if order.destination not in container_dict:
                container_dict[order.destination] = []
            for c in container_dict[order.destination]:
                if order.total_volume < c.volume_left:
                    c.orders.append(order)
                    break
            if order.total_volume <= self.container_volume:
                b = Container(self.container_volume, [order])
                b.orders.append(order)
                container_dict[order.destination].append(b)
            else:
                self.not_used_orders.append(order)
                container_dict.pop(order.destination, None)
